...
class Crawler:
    ...
    def craw(self):#爬虫的大脑，包括爬取网页，更新队列
        global g_queueURL
    ...
    def downloadAll(self):#调用多线程爬虫，在小雨线程最大值和没爬完队列之前，会增加线程
        global g_queueURL
        i = 0
    ...
    def download(self,url,tid): #调用多线程
        crawthread = CrawlerThread(url,tid)
        self.threadpool.append(crawthread)
        crawthread.start()

    def updateQueueURL(self):#完成一个深度爬虫之后，更新队列
        global g_queueURL
        global g_existURL
        newUrlList = []
    ...
    def getUrl(self,content): #从获取的网页中解析url
        link_list = re.findall('<a href="/wiki/([^:#=<>]*?)".*?</a>', content)#注意标签内部符号与字符串间的位置 注意空格
...
class CrawlerThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            g_mutex.relese()
            print('Fail downloading and saving',self.url)
            print(e)
            return None
        g_mutex.acquire()
        g_pages.append(html)
